          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group mb-2">
                ...
                <input type="text" v-model="plan.title" @change="v$.title.$touch" name="title" 
                id="name" placeholder="Enter your plan title" class="form-control" :class="{ 'border-danger': v$.title.$errors.length }" required>
                <div class="error-msg invalid-feedback d-block" v-for="error of v$.title.$errors" :key="error.$uid">{{ error.$message }}</div>
              </div>
            </div>
            <!-- <div class="mb-3">
          ...
          <input type="text" class="form-control form-control-lg" v-model="user.firstName" name="firstName" id="firstName"
            ...
            <div class="col-md-6">
              <div class="form-group mb-2">
                ...
                <input type="text" name="price" v-model="plan.price" @change="v$.price.$touch" id="price" :class="{ 'border-danger': v$.price.$errors.length }"
                 placeholder="Please Enter Price" class="form-control" required>
                <div class="error-msg invalid-feedback d-block" v-for="error of v$.price.$errors" :key="error.$uid">{{ error.$message }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group mb-2">
                ...
                <textarea v-model="plan.description" id="comments" @change="v$.description.$touch" class="form-control" name="comment"
                  placeholder="Enter your comment here..." :class="{ 'border-danger': v$.description.$errors.length }"></textarea>
                <div class="error-msg invalid-feedback d-block" v-for="error of v$.description.$errors" :key="error.$uid">{{ error.$message }}</div>
              </div>
            </div>
          </div>
...
const planCreate = async () => {
  ...
  const { data, pending }: any = await useFetch(`/api/plan`, {
        method: 'post',
        headers: header,
        body: plan.value,
      });
