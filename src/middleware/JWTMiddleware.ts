...
const JWTMiddleware = (req, res, next) => {
  ...
  try {
    ...
    if(decoded==null){
        return apiError(res, 401, false,"Token has expired please login again");
    }
    if(!decoded) {
      return apiError(res, 498, false,'Invalid token or may be Token has expired');
    ...
    if (error.name === 'TokenExpiredError') {
      return apiError(res, 401, false,"Token has expired");
      // return catchException(res, 'Token has expired');
      // You may want to return a response or throw an error here
    } else if (error.name === 'JsonWebTokenError') {
      return apiError(res, 401,false, "Invalid token");
      // return catchException(res, 'Invalid token');
      // You may want to return a response or throw an error here
    } else {
      return apiError(res, 401,false,"Authentication error");
